...
async function seedDatabase() {
  ...
  try {
    ...
    console.log("Clearing existing items in the 'items' collection...");
    await collection.deleteMany({});

    // Insert all items
    console.log(`Starting to seed ${menuItems.length} items...`);
    const result = await collection.insertMany(menuItems);

    console.log(`Success! ${result.insertedCount} items were added to your database.`);
  } catch (error) {
