        ...
        <div className="bg-[#F7FBE7]/90 backdrop-blur-2xl p-8 rounded-[2.5rem] shadow-2xl border border-white/60 overflow-hidden relative">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="group space-y-1">
              <label htmlFor="login-page-email" className="block text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2 group-focus-within:text-black transition-colors">Email Address</label>
              <div className="relative">
                ...
                <input id="login-page-email" aria-label="Email Address" type="email" required className="w-full pl-11 pr-4 py-3 bg-white/80 border-2 border-transparent rounded-2xl focus:border-[#BCE334] focus:bg-white outline-none text-sm font-bold shadow-sm transition-all text-black" placeholder="you@email.com" value={email} onChange={e => setEmail(e.target.value)} />
              </div>
            </div>
            <div className="group space-y-1">
              <label htmlFor="login-page-password" className="block text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2 group-focus-within:text-black transition-colors">Password</label>
              <div className="relative">
                ...
                <input id="login-page-password" aria-label="Password" type={showPassword ? "text" : "password"} required className="w-full pl-11 pr-12 py-3 bg-white/80 border-2 border-transparent rounded-2xl focus:border-[#BCE334] focus:bg-white outline-none text-sm font-bold shadow-sm transition-all text-black" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} />
                <button aria-label={showPassword ? "Hide password" : "Show password"} type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-black transition-colors">
